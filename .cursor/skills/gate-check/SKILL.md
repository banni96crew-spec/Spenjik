...
name: gate-check
description: >-
  Validate readiness to advance between development phases. Produces a PASSCONCERNSFAIL verdict with
  specific blockers and required artifacts. Use when user says 'are we ready to move to X', 'can we
  advance to production', 'check if we can start the next phase', 'pass the gate'.
---
...
- Treat user-provided arguments as: `[target-phase: systems-design | technical-setup | pre-production | production | polish | release] [--review full|lean|solo]`.
...
**Target phase:** `the first user-provided argument` (blank = auto-detect current stage, then validate next transition)
...
**Required Artifacts:**
- [ ] Engine chosen (.cursor/rules/ Technology Stack is not `[CHOOSE]`)
...
- [ ] Every genuinely new Foundation-layer decision not already owned by an
      accepted project PRD has an ADR in `docs/architecture/`. Zero ADRs is valid
      when `docs/technical-preferences.md` declares the modular PRD as the
      architecture authority and no uncovered decision exists.
...
**Quality Checks:**
- [ ] Core systems are covered by accepted owner PRDs and, only where the PRD
      does not own the decision, by ADRs
...
- [ ] Architecture traceability matrix has **zero Foundation layer gaps**
      (coverage may come from an accepted owner PRD or an ADR)
...
- [ ] Master architecture document exists at `docs/architecture/architecture.md`
- [ ] Every new Foundation-layer decision not already owned by an accepted PRD
      has an ADR; zero ADRs is valid when there are no uncovered decisions
- [ ] All existing Foundation and Core layer ADRs have status `Accepted`
      (not `Proposed`); PRD-owned decisions do not require duplicate ADRs
- [ ] Control manifest exists at `docs/architecture/control-manifest.md`
      (generated from accepted ADRs and project-owned technical rules; zero ADRs
      must not prevent generation when PRDs/preferences already own the rules)
...
Before generating the final verdict, run all four directors as **parallel specialist review passes** using the parallel gate protocol from `.cursor/docs/director-gates.md`. Start all four specialist passes before waiting for results.
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using asking the user.
...
- **No Difficulty Curve doc?** → Create `design/difficulty-curve.md` from the template at `.cursor/docs/templates/difficulty-curve.md` — or use `/quick-design "difficulty curve"` for a guided session.
- **No player journey map?** → Create `design/player-journey.md` from the template at `.cursor/docs/templates/player-journey.md` — or author it collaboratively using `/ux-design` Phase 2b.
