...
name: hotfix
description: >-
  Emergency fix workflow that bypasses normal sprint processes with a full audit trail. Creates hotfix
  branch, tracks approvals, and ensures the fix is backported correctly.
---
...
- Treat user-provided arguments as: `[bug-id or description]`.
...
If the check passes, Ask the user before creating the branch:
...
Use the specialist review pass to request sign-off in parallel:
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` as a specialist review pass with:
