...
Automate isolated development environments using git worktrees and tmux sessions with Cursor. Enables parallel work across multiple tasks, projects, and repositories.
...
|---------|-------------|---------|
| `review <ref>` | PR review session | `/psm review workflow#123` |
| `fix <ref>` | Issue fix session | `/psm fix workflow#42` |
| `feature <proj> <name>` | Feature development | `/psm feature workflow add-webhooks` |
| `list [project]` | List active sessions | `/psm list` |
| `attach <session>` | Attach to session | `/psm attach workflow:pr-123` |
| `kill <session>` | Kill session | `/psm kill workflow:pr-123` |
...
Supported formats:
- **Alias**: `workflow#123` (requires `~/.psm/projects.json`)
- **Full**: `owner/repo#123`
...
|------|--------------|--------------|
| PR Review | `psm:workflow:pr-123` | `~/.psm/worktrees/workflow/pr-123` |
| Issue Fix | `psm:workflow:issue-42` | `~/.psm/worktrees/workflow/issue-42` |
| Feature | `psm:workflow:feat-auth` | `~/.psm/worktrees/workflow/feat-auth` |
...
3. **Options**: --branch, --base, --no-Cursor Agent, --no-tmux, etc.
...
8. **Launch Cursor** (unless --no-Cursor Agent):
   ```bash
   ...
   tmux send-keys -t "psm:$project_alias:pr-$pr_number" "Cursor Agent --dangerously-skip-permissions" Enter

   # After Cursor Agent boots (PSM_AGENT_STARTUP_DELAY, default 5s), deliver the task.
   # Use -l (literal) so special characters are not misinterpreted by tmux.
   sleep "${PSM_AGENT_STARTUP_DELAY:-5}"
...
9. **Output session info**:
   ...
   Session ready!
     ...
     ID: workflow:pr-123
     Worktree: ~/.psm/worktrees/workflow/pr-123
     Tmux: psm:workflow:pr-123

   To attach: tmux attach -t psm:workflow:pr-123
   ```
...
6. **Update registry, create tmux, launch Cursor Agent**:
   Same as review, but pass issue context as the initial task prompt:
   ```bash
   tmux send-keys -t "psm:$project_alias:issue-$issue_number" "Cursor Agent --dangerously-skip-permissions" Enter
   # After Cursor Agent boots, deliver the task (see PSM_AGENT_STARTUP_DELAY):
...
4. **Create session, tmux, launch Cursor Agent** with feature context as initial prompt:
   ```bash
   tmux send-keys -t "psm:$project_alias:feat-$feature_name" "Cursor Agent --dangerously-skip-permissions" Enter
...
4. **Format output**:
   ...
   -------------------|---------|----------|---------------------------
   workflow:pr-123        | review  | active   | ~/.psm/worktrees/workflow/pr-123
   workflow:issue-42      | fix     | detached | ~/.psm/worktrees/workflow/issue-42
   ```
...
5. **Report**:
   ```
   Cleanup complete:
     Removed: workflow:pr-123 (merged)
     Removed: workflow:issue-42 (closed)
     Kept: workflow:feat-auth (active)
   ```
...
3. **Show status**:
   ```
   Current Session: workflow:pr-123
   Type: review
...
| `--path <path>` | Custom worktree root directory | `~/Workspace/workflow-worktrees/` |
