...
Writer Memory maintains context across Cursor Agent sessions for fiction writers. It tracks:

- **Characters (캐릭터)**: Emotional arcs (감정궤도), attitudes (태도), dialogue tone (대사톤), speech formality settings
...
| `speechFormality` | 말투 레벨 | Formality: 반말, 존댓말, 해체, 혼합 |
...
/writer-memory char update 새랑 speechFormality "해체"
...
| `intimacy_score` | Emotional closeness (1-10) |
...
| Check | Description |
|-------|-------------|
| **Speech Formality** | Does formality match? (반말/존댓말/해체) |
...
### With workflow notepad System
Writer Memory integrates with `.workflow/notepad.md`:
...
```
Delegate(role="architect", tier="opus", prompt="Analyze 새랑's arc across all scenes...")
```
...
/writer-memory char update 해랑 speechFormality "반말"
