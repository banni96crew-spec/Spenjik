...
name: team-narrative
description: >-
  Orchestrate the narrative team: coordinates narrative-director, writer, world-builder, and
  level-designer to create cohesive story content, world lore, and narrative-driven level design.
---
...
- Treat user-provided arguments as: `[narrative content description] [--review full|lean|solo]`.
...
Always provide full context in each agent's prompt (narrative brief, lore dependencies, character profiles). Run independent specialist passes in parallel where the pipeline allows it (e.g., Phase 2 agents can run simultaneously).
...
Delegate in parallel — issue all three specialist review passes simultaneously before waiting for any result:
...
specialist review passes spawned as a specialist review pass. Each specialist pass enforces the "May I write to [path]?"
