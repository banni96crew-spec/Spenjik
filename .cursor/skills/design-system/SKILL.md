...
name: design-system
description: >-
  Use when authoring or retrofitting a GDD for a single game system. Gathers existing design context,
  walks through required sections collaboratively, cross-references dependencies, and writes
...
- Treat user-provided arguments as: `<system-name> [--review full|lean|solo]`.
...
6. If yes: proceed to **Phase 2 (Gather Context)** as normal, but in **Phase 3**
   ...
   incomplete sections.
7. **Never overwrite existing section content.** Use edit operation to replace only
...
- Glob for and check if the file exists.
- If it **does not exist**: Write create it. Never attempt Edit on a file that may not exist.
- If it **already exists**: Edit update the relevant fields.
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   asking the user for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then Ask the user to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   asking the user. **NEVER use plain text. NEVER skip this step.**
...
7. **Write**: Edit replace the placeholder with the approved content.
   **CRITICAL**: Always include the section heading in the `old_string` to ensure
   ...
   same placeholder and the edit operation requires a unique match. Use this pattern:
   ```
...
completed section name. Glob for and check if the file exists — use Write to create
...
**Framing questions (ask BEFORE drafting)**: Ask the user with a multi-tab widget:
...
spawn `creative-director` as a specialist review pass:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents as a specialist review pass in parallel:
...
- Provide each specialist role with: system name, game concept summary, pillar set, dependency GDD excerpts, and the specific section being worked on
- Collect their findings before drafting
- Surface any disagreements between agents to the user by asking the user
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents as a specialist review pass in parallel:
...
- Present the specialists' proposals to the user for review by asking the user
...
For required systems: **spawn `art-director` as a specialist review pass** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` as a specialist review pass using gate **CD-GDD-ALIGN** (`.cursor/docs/director-gates.md`).
...
> **To validate this GDD, open a fresh Cursor agent chat and run:**
...
**When delegating via specialist review pass**:
...
- The main session presents the agent's output to the user by asking the user
- The user decides; the main session writes to file
...
2. **asking the user** at every decision point (Explain -> Capture pattern):
...
> open a fresh Cursor agent chat and run `/design-system [system-name]` — it will
