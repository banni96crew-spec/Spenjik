...
description: >-
  Holistic cross-GDD consistency and game design review. Reads all system GDDs simultaneously and
  checks for contradictions between them, stale references, ownership conflicts, formula
  incompatibilities, and game design theory violations (dominant strategies, economic imbalance,
  cognitive overload, pillar drift). Use after all MVP GDDs are written, before architecture begins.
---
...
- Treat user-provided arguments as: `[focus: full | consistency | design-theory | since-last-review]`.
...
**Focus:** `the first user-provided argument` (blank = `full`)
...
the same GDD inputs but produce separate reports. Run both as parallel specialist
review passes rather than waiting for Phase 2 to complete before starting Phase
3. Collect both results before writing the combined report.

**When running parallel specialist passes for Phase 2 and Phase 3, always pass:**
...
- The engine name and version from `.cursor/docs/technical-preferences.md` and `docs/engine-reference/[engine]/VERSION.md`

Do not rely on the SPECIALIST REVIEW PASS to re-read these files — it has its own context window and cannot access Phase 1 results unless they are explicitly passed in the specialist brief.
...
If any GDDs are flagged for revision, use a second asking the user:
...
After all file writes are complete, Ask the user for a closing widget.
...
3. **Offer options** via asking the user with three choices:
