---
name: release
description: 'Generic release assistant — analyzes repo release rules, caches them in .workflow/RELEASE_RULE.md, then guides the release'
---
...
A thin, repo-aware release assistant. On first run it inspects the project and CI to derive release rules, stores them in `.workflow/RELEASE_RULE.md` for future use, then walks you through a release using those rules.
...
Check whether `.workflow/RELEASE_RULE.md` exists.
...
Inspect the repo and answer the following. Write answers into `.workflow/RELEASE_RULE.md`.
...
### Step 2 — Write `.workflow/RELEASE_RULE.md`
...
- `.workflow/RELEASE_RULE.md` is a local cache. Commit it to your repo if you want to share the derived rules with your team, or add it to `.gitignore` if you prefer it stays local.
