...
name: team-audio
description: >-
  Orchestrate audio team: audio-director + sound-designer + technical-artist + gameplay-programmer for
...
- Treat user-provided arguments as: `[feature or area to design audio for] [--review full|lean|solo]`.
...
- Is the proposed audio integration idiomatic for Godot 4.6.2 (for example, built-in `AudioStreamPlayer` nodes versus separately approved middleware)?
...
5. **Save to** `design/audio/audio-[feature].md`.
   ...
   Note: If `design/audio/` does not exist, the specialist pass writing the document should create it (the directory will be created automatically when the file is written).
