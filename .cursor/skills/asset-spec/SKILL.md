...
name: asset-spec
description: >-
  Generate per-asset visual specifications and AI generation prompts from GDDs, level docs, or
  character profiles. Produces structured spec files and updates the master asset manifest. Use after
  art bible and GDDlevel design are approved, before production begins.
---
...
- Treat user-provided arguments as: `[system:<name> | level:<name> | character:<name>] [--review full|lean|solo]`.


If no argument is provided, check whether `design/assets/entity-inventory.md` exists:
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and Ask the user:
...
Present the full proposed inventory to the user in conversation. Then Ask the user:
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, Ask the user:
...
Spawn specialist agents based on review mode. **Issue all specialist review passes simultaneously — do not wait for one before starting the next.**

### Full mode — spawn in parallel:

**`art-director`** as a specialist review pass:
...
**`technical-artist`** as a specialist review pass:
