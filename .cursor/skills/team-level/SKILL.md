...
name: team-level
description: >-
  Orchestrate level design team: level-designer + narrative-director + world-builder + art-director +
  systems-designer + qa-tester for complete arealevel creation.
---
...
- Treat user-provided arguments as: `[level name or area to design] [--review full|lean|solo]`.


When this skill is invoked:

**Decision Points:** At each step transition, Ask the user to present
the user with the SPECIALIST REVIEW PASS's proposals as selectable options. Write the agent's
...
Use the specialist review pass to spawn each team member as a SPECIALIST REVIEW PASS:
...
Spawn all three agents simultaneously — issue all three specialist review passes before waiting for any result.
...
**Gate**: Ask the user to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
**Gate**: Ask the user to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Ask the user to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Ask the user to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
After all SPECIALIST REVIEW PASS outputs are collected, spawn `level-designer` as a specialist review pass to compile and write the final document:
- Pass: all SPECIALIST REVIEW PASS outputs (verbatim), the level brief, game pillars, relevant GDD sections
...
5. **Save to** `design/levels/[level-name].md` (handled by the level-designer SPECIALIST REVIEW PASS after user approval — see above).
...
to specialist review passes spawned as a specialist review pass. Each specialist pass enforces the "May I write to [path]?"
...
If any spawned agent (as a specialist review pass) returns BLOCKED, errors, or cannot complete:

1. **Surface immediately**: Report "[AgentName]: BLOCKED — [reason]" to the user before continuing to dependent phases
2. **Assess dependencies**: Check whether the blocked agent's output is required by subsequent phases. If yes, do not proceed past that dependency point without user input.
3. **Offer options** via asking the user with choices:
