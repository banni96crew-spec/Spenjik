...
name: test-flakiness
description: >-
  Detect non-deterministic (flaky) tests by reading CI run logs or test result history. Aggregates
  pass rates per test, identifies intermittent failures, recommends quarantine or fix, and maintains a
  flaky test registry. Best run during Polish phase or after multiple CI runs.
---
...
- Treat user-provided arguments as: `[ci-log-path | scan | registry]`.
