...
description: >-
  Validates completeness and consistency of the project architecture against all GDDs. Builds a
  traceability matrix mapping every GDD technical requirement to ADRs, identifies coverage gaps,
  detects cross-ADR conflicts, verifies engine compatibility consistency across all decisions, and
  produces a PASSCONCERNSFAIL verdict. Use as the architecture counterpart to design-review.
---
...
- Treat user-provided arguments as: `[focus: full | coverage | consistency | engine | single-gdd path/to/gdd.md]`.
...
After completing the engine audit above, spawn the **primary engine specialist** as a specialist review pass for a domain-expert second opinion:
...
If [B]: display the complete proposed systems-index section, then re-ask by asking the user.
...
5. **Ask the user for write approvals** — plain text "May I?" is not
