...
name: map-systems
description: >-
  Decompose a game concept into individual systems, map dependencies, prioritize design order, and
  create the systems index.
---
...
- Treat user-provided arguments as: `[next | system-name] [--review full|lean|solo]`.
...
Ask the user to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` as a specialist review pass using gate TD-SYSTEM-BOUNDARY (`.cursor/docs/director-gates.md`) before proceeding to priority assignment.**
...
Ask the user to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` as a specialist review pass using gate PR-SCOPE (`.cursor/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` as a specialist review pass using gate CD-SYSTEMS (`.cursor/docs/director-gates.md`).**
...
After the systems index is created (or after designing some systems), present next actions using asking the user:
...
> `design/gdd/systems-index.md`. Open a fresh Cursor agent chat to continue
