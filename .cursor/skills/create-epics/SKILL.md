...
name: create-epics
description: >-
  Translate approved GDDs + architecture into epics — one epic per architectural module. Defines
  scope, governing ADRs, engine risk, and untraced requirements. Does NOT break into stories — use the
  create-stories skill [epic-slug] after each epic is created.
---
...
- Treat user-provided arguments as: `[system-name | layer: foundation|core|feature|presentation | all] [--review full|lean|solo]`.
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` as a specialist review pass using gate **PR-EPIC** (`.cursor/docs/director-gates.md`).
