...
name: localize
description: >-
  Full localization pipeline: scan for hardcoded strings, extract and manage string tables, validate
  translations, generate translator briefings, run culturalsensitivity review, manage VO localization,
  test RTLplatform requirements, enforce string freeze, and report coverage.
---
...
- Treat user-provided arguments as: `[scan|extract|validate|status|brief|cultural-review|vo-pipeline|rtl-check|freeze|qa]`.
...
Spawn `localization-lead` as a specialist review pass. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
**Layout mirroring**
- Is RTL layout enabled through Godot `Control.layout_direction`?
...
Spawn `localization-lead` as a specialist review pass with:
