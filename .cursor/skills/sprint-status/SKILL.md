...
name: sprint-status
description: >-
  Fast sprint status check. Reads the current sprint plan, scans story files for status, and produces
  a concise progress snapshot with burndown assessment and emerging risks. Use at any time during a
  sprint for quick situational awareness. Use when user asks 'how is the sprint going', 'sprint
  update', 'show sprint progress'.
---
...
- Treat user-provided arguments as: `[sprint-number or blank for current]`.
...
**Argument:** `the first user-provided argument` (blank = use current sprint)
