...
name: smoke-check
description: >-
  Run the critical path smoke test gate before QA hand-off. Executes the automated test suite,
  verifies core functionality, and produces a PASSFAIL report. Use after a sprint's stories are
  implemented and before manual QA begins. A failed smoke check means the build is not ready for QA.
---
...
- Treat user-provided arguments as: `[sprint | quick | --platform pc|console|mobile|all]`.
...
Ask the user to batch-verify. Keep to at most 3 calls.
