...
name: quick-design
description: >-
  Lightweight design spec for small changes — tuning adjustments, minor mechanics, balance tweaks.
  Skips full GDD authoring when a system GDD already exists or the change is too small to warrant one.
  Produces a Quick Design Spec that embeds directly into story files.
---
...
- Treat user-provided arguments as: `[brief description of the change]`.
...
Present the inferred classification using asking the user:
...
Present the draft to the user in full. Then Ask the user:
