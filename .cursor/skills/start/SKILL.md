...
- Treat user-provided arguments as: `[no arguments]`.
...
- **Prompt**: "Welcome to Cursor Game Studios! Before I suggest anything, I'd like to understand where you're starting from. Where are you at with your game idea right now?"
- **Options**:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not asking the user (it's an open response).
2. Acknowledge the concept, then Ask the user to offer two paths:
...
After presenting the recommended path, Ask the user to ask the user which step they'd like to take first. Never auto-run the next skill.
