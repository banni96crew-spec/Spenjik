...
name: prototype
description: >-
  Concept prototype — validate the core idea is worth designing before writing GDDs. Run right after
  brainstorm and setup-engine. Routes to HTML, Engine, or Paper path based on game type. Produces a
  throwaway build and a PROCEEDPIVOTKILL verdict.
---
...
- Treat user-provided arguments as: `[concept-description] [--path html|engine|paper] [--review full|lean|solo] [--spike]`.
...
Otherwise, Ask the user to confirm intent before proceeding:
...
Read `.cursor/rules/` and `.cursor/docs/technical-preferences.md` for the engine and
language in use.
...
Assess which path best fits the hypothesis, then Ask the user with your
...
Read `.cursor/docs/templates/prototype-report.md` to get the report structure.
...
- `full` → spawn `creative-director` as a specialist review pass using gate **CD-PLAYTEST** if
...
2. **Choose path** — same asking the user widget as Phase 3 (HTML / Engine / Paper).
