...
name: skill-improve
description: >-
  Use when improving a Cursor skill through a test-fix-retest loop. Runs static checks, proposes
  targeted fixes, rewrites the skill, re-tests it, and keeps or reverts based on score change.
---
...
- Treat user-provided arguments as: `[skill-name]`.
...
- **Check 4 fail** → the skill writes, edits, creates, deletes, or updates files but has no ask-before-write language
- **Check 5 warn** → no follow-up or next-step section at the end
- **Check 6 fail/warn** → Cursor metadata is missing, mismatched with the folder name, or too vague for automatic selection
- **Check 7 warn** → input guidance is missing or does not match documented modes
