...
Meta-skill for managing workflow skills via CLI-like commands.
...
2. Scan user skills at `${AGENT_CONFIG_DIR:-~/.agent}/skills/cursor-learned/`
3. Scan project skills at `.workflow/skills/`
4. Parse YAML frontmatter for metadata
5. Display in organized table format:

```
BUILT-IN SKILLS (bundled with workflow):
| Name              | Description                    | Scope    |
|-------------------|--------------------------------|----------|
| visual-verdict    | Structured visual QA verdicts  | built-in |
| Ralph             | Ralph               | built-in |

USER SKILLS (~/.agent/skills/cursor-learned/):
...
**Built-in skill note:** Built-in skills are bundled with workflow and are discoverable/readable, but not removed or edited through `/skill remove` or `/skill edit`.
...
5. **Ask for scope:**
   - `user` → `${AGENT_CONFIG_DIR:-~/.agent}/skills/cursor-learned/<name>/SKILL.md`
   - `project` → `.workflow/skills/<name>/SKILL.md`
...
Argument hint (optional): <Tier> [message]
Scope (user/project): user

✓ Created skill at ~/.agent/skills/cursor-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `${AGENT_CONFIG_DIR:-~/.agent}/skills/cursor-learned/<name>/SKILL.md`
   - `.workflow/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `${AGENT_CONFIG_DIR:-~/.agent}/skills/cursor-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.agent/skills/cursor-learned/)
...
```
Skill: custom-logger
Scope: user (~/.agent/skills/cursor-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <Tier> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.agent/skills/cursor-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.agent/skills/cursor-learned/custom-logger/SKILL.md
...
**Behavior:**
1. **Scan both scopes:**
   - User skills: `${AGENT_CONFIG_DIR:-~/.agent}/skills/cursor-learned/`
   - Project skills: `.workflow/skills/`
...
User: yes
Assistant: ✓ Copied 'error-handler' to .workflow/skills/
...
```bash
# Check and create user-scope skills directory
USER_SKILLS_DIR="${AGENT_CONFIG_DIR:-$HOME/.agent}/skills/cursor-learned"
...
# Check and create project-scope skills directory
PROJECT_SKILLS_DIR=".workflow/skills"
...
```bash
# Scan user-scope skills
echo "=== user-scope SKILLS (~/.agent/skills/cursor-learned/) ==="
if [ -d "${AGENT_CONFIG_DIR:-$HOME/.agent}/skills/cursor-learned" ]; then
  USER_COUNT=$(find "${AGENT_CONFIG_DIR:-$HOME/.agent}/skills/cursor-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "${AGENT_CONFIG_DIR:-$HOME/.agent}/skills/cursor-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
echo ""
echo "=== project-scope SKILLS (.workflow/skills/) ==="
if [ -d ".workflow/skills" ]; then
  PROJECT_COUNT=$(find ".workflow/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".workflow/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
After scanning, use the a concise user question tool to offer these options:
...
Then ask for scope:
- **user-scope** (~/.agent/skills/cursor-learned/) - Available across all projects
- **project-scope** (.workflow/skills/) - Only for this project
...
**Automatic Application**: Cursor Agent detects triggers and applies skills automatically - no need to remember or search for solutions.

**Version Control**: project-scope skills (`.workflow/skills/`) are intended to be committed with your code so the whole team benefits. In linked worktrees, uncommitted skills remain local to that worktree and disappear if it is removed.

**Evolving Knowledge**: Skills improve over time as you discover better approaches and refine triggers.

**Reduced Token Usage**: Instead of re-solving the same problems, Cursor Agent applies known patterns efficiently.
...
✓ User skills directory exists: ~/.agent/skills/cursor-learned/
✓ Project skills directory exists: .workflow/skills/
...
- Use project-scope skills for codebase-specific knowledge
- Use user-scope skills for general patterns that apply everywhere
