...
name: setup-engine
description: >-
  Configure the project's game engine and version. Pins the engine and version in Cursor project rules
  and technical preferences, detects knowledge gaps, and populates engine reference docs via current
  documentation lookup when the version is beyond the LLM's training data.
---
...
- Treat user-provided arguments as: `[engine] | [engine version] | refresh | upgrade [old-version] [new-version] | no args for guided selection`.
...
**Question 1 — Prior experience** (ask this first, always, by asking the user):
...
**Question 2 — Target platform** (ask this second, always, by asking the user — platform eliminates or heavily weights engines before any other factor):
...
## 4. Update .cursor/rules/ Technology Stack
...
Record the choice. It determines the .cursor/rules/ template, naming conventions, specialist routing, and which agent is spawned for code files throughout the project.

---

Read `.cursor/rules/` and show the user the proposed Technology Stack changes.
Ask: "May I write these engine settings to `.cursor/rules/`?"
...
After updating .cursor/rules/, create or update `.cursor/docs/technical-preferences.md` with
...
## 8. Update .cursor/rules/ Import

Ask: "May I update the `@` import in `.cursor/rules/` to point to the new engine reference?"
...
- Always show the user what you're about to change before making .cursor/rules/ edits
- If web search returns ambiguous results, show the user and let them decide
- When the user chose **GDScript**: copy the GDScript .cursor/rules/ template from Appendix A1 exactly. NEVER add "C++ via GDExtension" to the Language field. GDScript projects may use GDExtension, but it is not a primary project language. The `godot-gdextension-specialist` in the routing table is available for when native extensions are needed — it does not make C++ a project language.
...
### A1. .cursor/rules/ Technology Stack Templates
