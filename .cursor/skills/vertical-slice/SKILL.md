...
name: vertical-slice
description: >-
  Pre-Production validation — build a production-quality end-to-end slice to confirm the full game
  loop is achievable before committing to Production. Use after GDDs, architecture, and UX specs are
  complete. Produces a PROCEED/PIVOT/KILL verdict for the Pre-Production → Production transition.
---
...
- `.cursor/rules/` — tech stack and engine
...
Read `.cursor/docs/templates/vertical-slice-report.md` to get the report structure.
...
- `full` → spawn `creative-director` as a specialist review pass using gate **CD-PLAYTEST**
