...
```
Delegate(role="explore", tier="haiku", prompt="List all directories recursively. Exclude: node_modules, .git, dist, build, __pycache__, .venv, coverage, .next, .nuxt")
```

### Step 2: Create Work Plan

Generate todo items for each directory, organized by directory depth:

```
Tier 0: / (root)
Tier 1: /src, /docs, /tests
Tier 2: /src/components, /src/utils, /docs/api
...
```

### Step 3: Generate Tier by Tier

**IMPORTANT**: Generate parent tiers before child tiers to ensure parent references are valid.
...
1. **Same-Tier directories**: Process in parallel
2. **Different tiers**: Sequential (parent first)
