---
name: trace
description: 'Evidence-driven tracing lane that orchestrates competing tracer hypotheses in Cursor Agent built-in team mode'
metadata:
...
This is the orchestration layer on top of the built-in `tracer` agent. The goal is to make tracing feel like a reusable workflow operating lane: restate the observation, generate competing explanations, gather evidence in parallel, rank the explanations, and propose the next probe that would collapse uncertainty fastest.
...
Use **Cursor Agent built-in team mode** for `/trace`.
...
3. “Create one tracer lane per hypothesis using Cursor Agent built-in team mode.”
