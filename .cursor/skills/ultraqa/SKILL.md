...
1. **RUN QA**: Execute verification based on goal type
   ...
   - `--interactive`: Use qa-tester for interactive CLI/service testing:
     ```
     Delegate(role="qa-tester", tier="sonnet", prompt="TEST:
...
3. **ARCHITECT DIAGNOSIS**: Spawn architect to analyze failure
   ...
   ```
   Delegate(role="architect", tier="opus", prompt="DIAGNOSE FAILURE:
   Goal: [goal type]
...
4. **FIX ISSUES**: Apply architect's recommendations
   ...
   ```
   Delegate(role="executor", tier="sonnet", prompt="FIX:
...
Track state in `.workflow/ultraqa-state.json`:
...
rm -f .workflow/state/ultraqa-state.json
```
