---
name: team
description: 'N coordinated agents on shared task list using Cursor native teams'
metadata:
...
Spawn N coordinated agents working on a shared task list using Cursor's native team tools. Replaces the legacy `/swarm` skill (SQLite-based) with built-in team management, inter-agent messaging, and task dependencies -- no external dependencies required.
...
- **agent-type** - workflow agent to spawn for the `team-exec` stage (e.g., executor, debugger, designer, codex, gemini). Optional; defaults to stage-aware routing. Use `codex` to spawn Codex CLI workers or `gemini` for Gemini CLI workers (requires respective CLIs installed). See Stage Agent Routing below.
- **task** - broad task to decompose and distribute among teammates
...
User: "/team 3:executor fix all TypeScript errors"
      ...
      [TEAM ORCHESTRATOR (Lead)]
              ...
              |
              +-- Cursor team worker "worker-1" in "fix-ts-errors" x 3
              ...
              +-- Completion
                      ...
                      -> rm .workflow/state/team-state.json
```

**Storage layout (managed by Cursor):**
...
Team is the workflow authority for parallel, staged execution. Use the deterministic conflict policies `refuse`, `adopt_existing`, and `artifact_only` rather than non-deterministic warning handling. If a task mentions Cursor `/goal`, Ralph, UltraQA, or artifact-only Ultragoal, keep Team as the primary loop authority unless the leader explicitly hands off. Use `/goal` only as a documented native Cursor handoff target or as visible evidence from the lead session; do not claim the `/goal` evaluator independently runs commands, reads files, or replaces `team-verify` / `team-fix`. Artifact-only Ultragoal references should be treated as durable goal ledger/checkpoint/evidence artifacts, not as worker execution by themselves.
...
2. **Specialist agents complement executor agents.** Route analysis/review to architect/critic Cursor Agent agents and UI work to designer agents. Tmux CLI workers are one-shot and don't participate in team communication.
...
4. **risk tier escalates review.** Security-sensitive or >20 file changes must include `security-reviewer` + `code-reviewer` (opus) in `team-verify`.
...
The lead writes handoffs to `.workflow/handoffs/<stage-name>.md`.
...
3. **On team cancellation, handoffs survive** in `.workflow/handoffs/` for session resume. They are not deleted by `TeamDelete`.
...
- **Resume:** restart from the last non-terminal stage using staged state + live task status. Read `.workflow/handoffs/` to recover stage transition context.
- **Cancel:** `/cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.workflow/handoffs/` are preserved for potential resume.
...
- Extract **agent-type**, validate it maps to a known workflow subagent
...
{
  "team_name": "fix-ts-errors",
  "team_file_path": "~/.agent/teams/fix-ts-errors/config.json",
...
Write workflow state using the `state_write` MCP tool for proper session-scoped persistence:
...
state_write(mode="team", active=true, current_phase="team-plan", state={
  ...
  "fix_loop_count": 0,
  "max_fix_loops": 3,
  "linked_persistence_loop": false,
  "stage_history": "team-plan"
})
```

> **Note:** The MCP `state_write` tool transports all values as strings. Consumers must coerce `agent_count`, `fix_loop_count`, `max_fix_loops` to numbers and `linked_persistence_loop` to boolean when reading state.
...
| `linked_persistence_loop`   | boolean | Whether team is linked to a Ralph persistence loop                                      |
...
On every stage transition, update workflow state:
...
4. **Delete team** -- Call `TeamDelete` to clean up:
   ...
   ```
5. **Clean workflow state** -- Remove `.workflow/state/team-state.json`
...
- `agent_worker`: Emphasize strict TaskList/TaskUpdate/SendMessage loop and no orchestration commands.
...
```bash
node "${AGENT_PLUGIN_ROOT}/scripts/cleanup-orphans.mjs" --team-name fix-ts-errors
```
...
The team skill supports **hybrid execution** combining Cursor Agent agent teammates with external CLI workers (Codex CLI and Gemini CLI). Both types can make code changes -- they differ in capabilities and cost. These are standalone CLI tools, not MCP servers.
...
| --------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `agent_worker` | Cursor Agent agent           | Full Cursor tool access (Read/Write/Edit/Bash/Task). Best for tasks needing Cursor Agent's reasoning + iterative tool use.                                                                  |
...
**Key difference from Cursor Agent teammates:**

- CLI workers operate via tmux, not Cursor's tool system
...
| -------------------------------- | ------------------------------ | --------------------------------------------------- |
| Iterative multi-step work        | Cursor Agent teammate                | Needs tool-mediated iteration + team communication  |
| Code review / security audit     | CLI worker or specialist agent | Autonomous execution, good at structured analysis   |
| Architecture analysis / planning | architect Cursor Agent agent         | Strong analytical reasoning with codebase access    |
| Refactoring (well-scoped)        | CLI worker or executor agent   | Autonomous execution, good at structured transforms |
| UI/frontend implementation       | designer Cursor Agent agent          | Design expertise, framework idioms                  |
| Large-scale documentation        | writer Cursor Agent agent            | Writing expertise + large context for consistency   |
| Build/test iteration loops       | Cursor Agent teammate                | Needs Bash tool + iterative fix cycles              |
| Tasks needing team coordination  | Cursor Agent teammate                | Needs SendMessage for status updates                |
...
Task decomposition:
#1 [codex_worker] Security review of current auth code -> output to .workflow/research/auth-security.md
#2 [codex_worker] Refactor auth/login.ts and auth/session.ts (uses #1 findings)
#3 [agent_worker:designer] Redesign auth UI components (login form, session indicator)
#4 [agent_worker] Update auth tests + fix integration issues
#5 [gemini_worker] Final code review of all changes
```

The lead runs #1 (Codex security analysis), then #2 and #3 in parallel (Codex refactors backend, designer agent redesigns frontend), then #4 (Cursor Agent teammate handles test iteration), then #5 (Gemini final review).

### Pre-flight Analysis (Optional)

For large ambiguous tasks, run analysis before team creation:

1. Spawn `Cursor-supported delegation to a planner-capable worker` with task description + codebase context
...
This approach complements the existing `SendMessage`-based communication by providing a pull-based mechanism for MCP workers that cannot use Cursor's team messaging tools.
...
4. If needed, spawn a replacement teammate with `a Cursor-supported replacement teammate`
...
1. Check `${AGENT_CONFIG_DIR:-~/.agent}/teams/` for teams matching the task slug
...
| ----------------------- | ------------------------------------------------------------------ | -------------------------------------- |
| **Storage**             | JSON files in `~/.agent/teams/` and `~/.agent/tasks/`            | SQLite in `.workflow/state/swarm.db`        |
...
| **Heartbeat**           | Automatic idle notifications from Cursor                      | Manual heartbeat table + polling       |
...
**When to use Team over Swarm:** Always prefer `/team` for new work. It uses Cursor's built-in infrastructure, requires no external dependencies, supports inter-agent communication, and has task dependency management.

## Cancellation

The `/cancel` skill handles team cleanup:

1. Read team state via `state_read(mode="team")` to get `team_name` and `linked_persistence_loop`
...
6. If `linked_persistence_loop` is true, also clear Ralph: `state_clear(mode="Ralph")`
...
If teammates are unresponsive, `TeamDelete` may fail. In that case, the cancel skill should wait briefly and retry, or inform the user to manually clean up `~/.agent/teams/{team_name}/` and `~/.agent/tasks/{team_name}/`.

## Runtime V2 (Event-Driven)

When `WORKFLOW_RUNTIME_V2=1` is set, the team runtime uses an event-driven architecture instead of the legacy done.json polling watchdog:
...
- **Event log**: All team events are appended to `.workflow/state/team/{teamName}/events.jsonl`
- **Worker status files**: Workers write status to `.workflow/state/team/{teamName}/workers/{name}/status.json`
...
When `WORKFLOW_TEAM_SCALING_ENABLED=1` is set, the team supports mid-session scaling:
...
Optional settings live in `.agent/workflow.jsonc` (project) or `~/.config/agent-workflow/config.jsonc` (user). Project values override user values; `WORKFLOW_TEAM_ROLE_OVERRIDES` (env JSON) supersedes both.
...
- **ops.defaultAgentType** - CLI provider when a `/team` invocation does not specify one (`Cursor Agent` | `codex` | `gemini` | `grok` | `cursor`, default: `Cursor Agent`)
...
> **Note:** Team members do not have a hardcoded model default. Each teammate is a separate Cursor session that inherits the user's configured model. Since teammates can spawn their own subagents, the session model acts as the orchestration layer while subagents can use any model tier.
...
| Role            | Provider        | Model                     |
| --------------- | --------------- | ------------------------- |
| `orchestrator`  | Cursor Agent (pinned) | inherits invoking session |
| `planner`       | Cursor Agent          | `HIGH` (opus)             |
| `analyst`       | Cursor Agent          | `HIGH` (opus)             |
| `executor`      | Cursor Agent          | `MEDIUM` (sonnet)         |
...
- **provider** — `"Cursor Agent" | "codex" | "gemini" | "grok" | "cursor"`. Omitted → defaults to `Cursor Agent`.
- **model** — tier name (`"HIGH" | "MEDIUM" | "LOW"`) or an explicit model ID. Tiers resolve through `routing.tierModels`.
- **agent** — optional Cursor Agent agent name (e.g. `"critic"`, `"executor"`). Only honored when the resolved provider is `Cursor Agent`.

`orchestrator` is pinned to `Cursor Agent`; only `model` is user-configurable. Any other key on `orchestrator` is rejected by the validator.
...
```bash
WORKFLOW_TEAM_ROLE_OVERRIDES='{"critic":{"provider":"codex"},"code-reviewer":{"provider":"gemini"}}'
```

Precedence: `WORKFLOW_TEAM_ROLE_OVERRIDES` > `.agent/workflow.jsonc` (project) > `~/.config/agent-workflow/config.jsonc` (user) > built-in defaults. Invalid JSON logs a warning and is ignored — env overrides are best-effort and never abort the run.

### Fallback when a CLI is missing

If the CLI for a configured provider is absent from `PATH` at spawn time, `buildLaunchArgs()` throws, the team lead emits a visible `SendMessage` warning, and the runtime falls back to a deterministic Cursor Agent assignment pre-computed by `buildResolvedRoutingSnapshot` (same tier + same agent, `provider: "Cursor Agent"`). Fallback is loud by design — silent fallback is a test failure. Probe provider availability with `omc doctor --team-routing`.
...
An empty `team.roleRouting` preserves pre-patch behavior: every worker is Cursor Agent, model tiers follow `routing.tierModels`, and `/team 3:executor ...` still spawns three Cursor Agent Sonnet executors.

## State Cleanup

On successful completion:

1. `TeamDelete` handles all Cursor state:
   - Removes `~/.agent/teams/{team_name}/` (config)
   - Removes `~/.agent/tasks/{team_name}/` (all task files + lock)
2. workflow state cleanup via MCP tools:
   ```
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.workflow/worktrees/{team}/{worker}` with branch `workflow-team/{teamName}/{workerName}`.
...
11. **CLI workers are one-shot, not persistent** -- Tmux CLI workers have full filesystem access and CAN make code changes. However, they run as autonomous one-shot jobs -- they cannot use TaskList/TaskUpdate/SendMessage. The lead must manage their lifecycle: write prompt_file, spawn CLI worker, read output_file, mark task complete. They don't participate in team communication like Cursor Agent teammates do.

## Parallel session caveats

- **Multi-repo workspace anchor:** drop a `.workflow-workspace` marker at the parent directory so multiple sessions across sub-repos share one `.workflow/`. Resolution order: `WORKFLOW_STATE_DIR > .workflow-workspace > git > cwd`. See `docs/REFERENCE.md`.
- **Session id source:** WORKFLOW_SESSION_ID env var wins in CLI contexts; hook payload data.session_id wins in hook contexts.
- **Plan id (when applicable):** Team state is session-scoped. Team handoffs at `.workflow/handoffs/` are shared by design (see Wave G in the workspace plan).
