...
description: >-
  Validate that a story file is implementation-ready. Checks for embedded GDD requirements, ADR
  references, engine notes, clear acceptance criteria, and no open design questions. Produces READY /
  NEEDS WORK / BLOCKED verdict with specific gaps. Use when user says 'is this story ready', 'can I
  start on this story', 'is story X ready to implement'.
---
...
- Treat user-provided arguments as: `[story-file-path or 'all' or 'sprint']`.
...
**Scope:** `the first user-provided argument` (blank = ask user via asking the user)
...
- [ ] **Referenced assets exist**: Scan the story text for asset path patterns
  ...
  - For each asset path found: Glob for and check whether the file exists.
...
in conversation. Do not use Write or edit operations — the user (or
...
Spawn `qa-lead` as a specialist review pass using gate **QL-STORY-READY** (`.cursor/docs/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user by asking the user:
