...
name: team-live-ops
description: >-
  Orchestrate the live-ops team for post-launch content planning: coordinates live-ops-designer,
  economy-designer, analytics-engineer, community-manager, writer, and narrative-director to design
...
- Treat user-provided arguments as: `[season name or event description] [--review full|lean|solo]`.
...
Always provide full context in each agent's prompt (game concept path, existing season docs, ethics policy path, current economy state). Run independent specialist passes in parallel where the pipeline allows it (Phases 3 and 4 can run simultaneously).
