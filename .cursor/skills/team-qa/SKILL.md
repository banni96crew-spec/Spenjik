...
name: team-qa
description: >-
  Orchestrate the QA team through a full testing cycle. Coordinates qa-lead (strategy + test plan) and
  qa-tester (test case writing + bug reporting) to produce a complete QA package for a sprint or
  feature. Covers: test plan generation, test case writing, smoke check gate, manual QA execution, and
  sign-off report.
---
...
- Treat user-provided arguments as: `[sprint | feature: system-name] [--review full|lean|solo]`.
...
Spawn `qa-lead` as a specialist review pass to review all in-scope stories and produce a QA strategy.
...
Spawn `qa-tester` as a specialist review pass for each story (run in parallel where possible), providing:
- The story file path
...
After each FAIL result: Ask the user to collect the failure description, then spawn `qa-tester` as a specialist review pass to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` as a specialist review pass to produce the sign-off report using all results from Phases 4–6.
