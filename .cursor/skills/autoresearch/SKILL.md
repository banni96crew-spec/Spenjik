...
- You need durable experiment logs under `.workflow/autoresearch/`
- You want a supported path for periodic reruns via Cursor native cron
</Use_When>
...
<Required_Artifacts>
Canonical persistent storage lives under `.workflow/autoresearch/<mission-slug>/` and/or `.workflow/logs/autoresearch/<run-id>/`.
...
```text
.workflow/autoresearch/<mission-slug>/
  mission.md
  evaluator.json
  runs/<run-id>/
    evaluations/
...
<Cron_Integration>
Cursor native cron is a supported integration point for periodic mission enhancement. In v1, prefer documenting/configuring cron inputs over building a large scheduler UI.
