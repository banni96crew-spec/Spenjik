...
Configure Model Context Protocol (MCP) servers to extend Cursor's capabilities with external tools like web search, file system access, and GitHub integration.

## Overview

MCP servers provide additional tools that Cursor agents can use. This skill helps you configure popular MCP servers using the `Cursor Agent mcp add` command-line interface.

## Step 1: Choose a Setup Path

Use **a concise user question** with **one question at a time** and **no more than 3 options per question**. Recent Cursor builds reject larger option payloads as invalid tool parameters, so keep the MCP selection flow staged.
...
**Options:**
1. **Recommended starter setup** - Fast path for the most common workflow MCP additions
...
Ask a follow-up **a concise user question**:
...
If the user chooses **More server choices**, ask one more **a concise user question**:
...
Use the `Cursor Agent mcp add` command to configure each MCP server. The CLI automatically handles settings.json updates and merging.

### Context7 Configuration:
```bash
Cursor Agent mcp add context7 -- npx -y @upstash/context7-mcp
```

### Exa Web Search Configuration:
```bash
Cursor Agent mcp add -e EXA_API_KEY=<user-provided-key> exa -- npx -y exa-mcp-server
```

### Filesystem Configuration:
```bash
Cursor Agent mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem <allowed-directories>
```
...
```bash
Cursor Agent mcp add -e GITHUB_PERSONAL_ACCESS_TOKEN=<user-provided-token> github -- docker run -i --rm -e GITHUB_PERSONAL_ACCESS_TOKEN ghcr.io/github/github-mcp-server
```

**Option 2: HTTP (remote)**
```bash
Cursor Agent mcp add --transport http github https://api.githubcopilot.com/mcp/
```
...
3. Run `Cursor Agent mcp list` to verify configuration
...
TROUBLESHOOTING:
- If MCP servers don't appear, run `Cursor Agent mcp list` to check status
...
- Add more servers: /mcp-setup or `Cursor Agent mcp add ...`
- List servers: `Cursor Agent mcp list`
- Remove a server: `Cursor Agent mcp remove <server-name>`
```
...
Then construct and run the appropriate `Cursor Agent mcp add` command:
...
Cursor Agent mcp add -e KEY1=value1 -e KEY2=value2 <server-name> -- <command> [args...]
```

**For HTTP servers:**
```bash
# Basic HTTP server
Cursor Agent mcp add --transport http <server-name> <url>

# HTTP server with headers
Cursor Agent mcp add --transport http --header "Authorization: Bearer <token>" <server-name> <url>
```

### Company-context convention

If the custom server is meant to provide organization-specific reference material to workflow workflows, prefer a single tool named `get_company_context` that returns markdown via `{ context: string }`.

Example local registration:

```bash
Cursor Agent mcp add company-context -- node examples/vendor-mcp-server/server.mjs
```

Then point workflow at the full tool name in `.agent/workflow.jsonc` or `~/.config/agent-workflow/config.jsonc`:
