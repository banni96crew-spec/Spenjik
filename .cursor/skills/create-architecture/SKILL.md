...
description: >-
  Guided, section-by-section authoring of the master architecture document for the game. Reads all
  GDDs, the systems index, existing ADRs, and the engine reference library to produce a complete
  architecture blueprint before any code is written. Engine-version-aware: flags knowledge gaps and
  validates decisions against the pinned engine version.
---
...
- Treat user-provided arguments as: `[focus-area: full | layers | data-flow | api-boundaries | adr-audit] [--review full|lean|solo]`.
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then Ask the user:
...
**Step 2 — Spawn `lead-programmer` as a specialist review pass using gate LP-FEASIBILITY (`.cursor/docs/director-gates.md`):**
...
Show the proposed Document Status block inline, then Ask the user:
