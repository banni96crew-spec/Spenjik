...
<Tool_Usage>
- Use Cursor-supported delegation, for example `Delegate(subagent_type="executor", model="haiku", ...)` for simple changes
- Use Cursor-supported delegation, for example `Delegate(subagent_type="executor", model="sonnet", ...)` for standard work
- Use Cursor-supported delegation, for example `Delegate(subagent_type="executor", model="opus", ...)` for complex work
...
```
Delegate(role="executor", tier="haiku", prompt="Add missing type export for Config interface")
Delegate(role="executor", tier="sonnet", prompt="Implement the /api/users endpoint with validation")
Delegate(role="executor", tier="sonnet", prompt="Add integration tests for the auth middleware")
```
...
```
Delegate(role="executor", tier="sonnet", prompt="npm install && npm run build", run_in_background=true)
Delegate(role="executor", tier="haiku", prompt="Update the README with new API endpoints")
```
...
```
result1 = Delegate(role="executor", prompt="Add type export")  # wait...
result2 = Delegate(role="executor", prompt="Implement endpoint")     # wait...
result3 = Delegate(role="executor", prompt="Add tests")              # wait...
```
...
```
Delegate(role="executor", tier="opus", prompt="Add a missing semicolon")
```
