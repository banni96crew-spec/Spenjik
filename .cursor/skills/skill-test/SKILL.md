...
name: skill-test
description: >-
  Validate skill files for structural compliance and behavioral correctness. Three modes: static
  (linter), spec (behavioral), audit (coverage report).
---
...
- Treat user-provided arguments as: `static [skill-name | all] | spec [skill-name] | category [skill-name | all] | audit`.
...
|------|---------|---------|------------|
| `static` | `/skill-test static [name\|all]` | Structural linter — Cursor Skill compliance checks | Low (~1k/skill) |
...
- `static [name]` → run Cursor Skill structural checks on one skill
- `static all` → run Cursor Skill structural checks on all skills (Glob `.cursor/skills/*/SKILL.md`)
...
For each skill being tested, read its `SKILL.md` fully and run all checks:
...
**WARN** if frontmatter contains legacy non-Cursor fields such as
`argument-hint`, `user-invocable`, `allowed-tools`, `model`, `agent`, or
`context`. Cursor Skills should keep invocation details and workflow context in
the Markdown body, not in metadata.
...
**FAIL** if the skill body writes, edits, creates, deletes, or updates files but
has no ask-before-write language.
...
### Check 6 — Cursor Metadata Hygiene
The YAML frontmatter should stay small and Cursor-compatible:
- `name:` is a lowercase hyphenated identifier that matches the folder name
- `description:` explains both what the skill does and when Cursor should use it
- Optional Cursor fields such as `paths` or `disable-model-invocation` appear
  only when they are intentionally needed

**FAIL** if `name` is missing, mismatched, or not lowercase hyphenated.
**WARN** if the description is too vague to drive automatic selection.

### Check 7 — Input Guidance
If the skill supports arguments or modes, the body should document them in a
short `## Inputs` section or in the first parse/validation phase. Cross-reference
the documented inputs against the first phase's "Parse Arguments" section.

**WARN** if documented modes do not match the input guidance.
...
Check 6 — Cursor Metadata:       PASS
Check 7 — Input Guidance:        PASS
