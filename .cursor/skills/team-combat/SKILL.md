...
name: team-combat
description: >-
  Orchestrate the combat team: coordinates game-designer, gameplay-programmer, ai-programmer,
  technical-artist, sound-designer, and qa-tester to design, implement, and validate a combat feature
  end-to-end.
---
...
- Treat user-provided arguments as: `[combat feature description] [--review full|lean|solo]`.
...
Then stop immediately without spawning any specialist review passes or reading any files.
...
Always provide full context in each agent's prompt (design doc path, relevant code files, constraints). Run independent specialist passes in parallel where the pipeline allows it (e.g., Phase 3 agents can run simultaneously).
...
- Is the class/node/resource structure idiomatic for Godot 4.6.2 and consistent with the approved scene and state architecture?
...
delegated to specialist review passes spawned as a specialist review pass. Each specialist pass enforces the
