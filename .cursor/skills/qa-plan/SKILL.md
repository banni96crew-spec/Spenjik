...
name: qa-plan
description: >-
  Generate a QA test plan for a sprint or feature. Reads GDDs and story files, classifies stories by
  test type (LogicIntegrationVisualUI), and produces a structured test plan covering automated tests
  required, manual test cases, smoke test scope, and playtest sign-off requirements. Use before sprint
  begins or when starting a major feature.
---
...
- Treat user-provided arguments as: `[sprint | feature: system-name | story: path]`.
...
**Argument:** `user-provided arguments` (blank = ask user via asking the user)
...
then ask two questions together using asking the user:
