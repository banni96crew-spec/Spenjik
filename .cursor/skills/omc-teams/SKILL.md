---
name: omc-teams
description: 'CLI-team runtime for Cursor Agent, codex, or gemini workers in tmux panes when you need process-based parallel execution'
metadata:
  aliases: '[]'
---

# workflow Teams Skill
...
Spawn N CLI worker processes in tmux panes to execute tasks in parallel. Supports `Cursor Agent`, `codex`, and `gemini` agent types.
...
```bash
/omc-teams N:Cursor Agent "task description"
...
- **agent-type** - `Cursor Agent` (Cursor Agent CLI), `codex` (OpenAI Codex CLI), or `gemini` (Google Gemini CLI)
- **task** - Task description to distribute across all workers

### Examples

```bash
/omc-teams 2:Cursor Agent "implement auth module with tests"
...
- **Cursor Agent** CLI: `npm install -g @anthropic-ai/host-agent`
...
- `N` — worker count (1–10)
- `agent-type` — `Cursor Agent|codex|gemini`
- `task` — task description

Validate before decomposing or running anything:

- Reject unsupported agent types up front. `/omc-teams` only supports **`Cursor Agent`**, **`codex`**, and **`gemini`**.
- If the user asks for an unsupported type such as `expert`, explain that `/omc-teams` launches external CLI workers only.
- For native Cursor team agents/roles, direct them to **`/team`** instead.
...
- If the task references a plan artifact under one repo (for example
  `tool/.workflow/plans/task-1200-gwd-gifs.md`) and target paths in sibling repos
...
- Do not anchor the launch cwd to only the repo containing `.workflow/plans/...` when
  target repos are siblings; that strands `codex`, `Cursor Agent`, and `gemini` workers in
...
```bash
omc team <N>:<Cursor Agent|codex|gemini> "<task>"
```
...
```bash
omc team <N>:<Cursor Agent|codex|gemini> "<task with absolute plan path and explicit repo paths>" --cwd <workspace-root>
```
...
- `WORKFLOW_run_team_start`
- `WORKFLOW_run_team_status`
- `WORKFLOW_run_team_wait`
- `WORKFLOW_run_team_cleanup`
...
| `cmux surface detected`      | Running inside cmux without `$TMUX` | Use the normal `omc team ...` flow; workflow will create native cmux worker splits      |
| `Unsupported agent type`     | Requested agent is not Cursor Agent/codex/gemini/grok/cursor | Use `Cursor Agent`, `codex`, `gemini`, `grok`, or `cursor`; for native Cursor agents use `/team` |
...
| ------------ | ----------------------------------------- | ---------------------------------------------------- |
| Worker type  | Cursor native team agents            | Cursor Agent / codex / gemini CLI processes in tmux        |
...
| Use when     | You want Cursor Agent-native team orchestration | You want external CLI worker execution               |
