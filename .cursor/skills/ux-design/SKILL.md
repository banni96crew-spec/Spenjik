...
name: ux-design
description: >-
  Use when authoring a UX spec for a screen, flow, HUD, or interaction pattern. Reads the game concept,
  player journey, and relevant GDDs to provide context-aware design guidance and produce UX documents
...
- Treat user-provided arguments as: `[screen/flow name] or 'hud' or 'patterns'`.
...
> "Player journey map not yet created. Template available at `.cursor/docs/templates/player-journey.md`. Run `/ux-design` Phase 2b or create it manually to establish player context for this screen."
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then Ask the user to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Ask the user:
...
7. **Write**: Ask the user: "May I write the [section name] section to `[filepath]`?"
...
- Ask the user to capture the choice:
...
Work through patterns in groups. Ask the user:
...
When delegating to another agent via the specialist review pass:
