...
description: >-
  Use after architecture is complete to produce a flat actionable rules sheet for programmers — what
  they must do and must never do per system and layer. Extracts rules from Accepted ADRs, technical
...
- Treat user-provided arguments as: `[update — regenerate from current ADRs]`.
...
Spawn `technical-director` as a specialist review pass using gate **TD-MANIFEST** (`.cursor/docs/director-gates.md`).
...
- **CONCERNS** → surface by asking the user with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
