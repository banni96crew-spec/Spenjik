...
Set up workflow notification integrations so you're alerted when sessions end, need input, or complete background tasks.
...
- If no provider is specified, use a concise user question:
...
Set up Telegram notifications so workflow can message you when sessions end, need input, or complete background tasks.

### How This Skill Works

This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with a concise user question. Write the result to `${AGENT_CONFIG_DIR:-~/.agent}/.workflow-config.json`.
...
2. Send /newbot
3. Choose a name (e.g., "My workflow Notifier")
4. Choose a username (e.g., "my_workflow_bot")
...
Use a concise user question with multiSelect:

**Question:** "Which events should trigger Telegram notifications?"

**Options (multiSelect: true):**
1. **Session end (Recommended)** - When a Cursor Agent session finishes
2. **Input needed** - When Cursor Agent is waiting for your response (great for long-running tasks)
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "text=workflow test notification - Telegram is configured!")
...
Config saved to: ~/.agent/.workflow-config.json

You can also set these via environment variables:
  WORKFLOW_TELEGRAM_BOT_TOKEN=123456789:ABCdefGHI...
  WORKFLOW_TELEGRAM_CHAT_ID=123456789
...
```bash
export WORKFLOW_TELEGRAM_BOT_TOKEN="123456789:ABCdefGHIjklMNOpqrsTUVwxyz"
export WORKFLOW_TELEGRAM_CHAT_ID="123456789"
```

Env vars are auto-detected by the notification system without needing `.workflow-config.json`.

---

## Discord Setup

Set up Discord notifications so workflow can ping you when sessions end, need input, or complete background tasks.
...
**Options:**
1. **workflow (default)** - Display as "workflow"
2. **Cursor** - Display as "Cursor"
...
Build the notifications object with the collected values and merge into `.workflow-config.json` using jq:
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d "{\"content\": \"${MENTION:+$MENTION\\n}workflow test notification - Discord is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  WORKFLOW_DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/...
  WORKFLOW_DISCORD_MENTION=<@1465264645320474637>
...
**Webhook method:**
```bash
export WORKFLOW_DISCORD_WEBHOOK_URL="https://discord.com/api/webhooks/..."
export WORKFLOW_DISCORD_MENTION="<@1465264645320474637>"  # optional
```

**Bot API method:**
```bash
export WORKFLOW_DISCORD_NOTIFIER_BOT_TOKEN="your-bot-token"
export WORKFLOW_DISCORD_NOTIFIER_CHANNEL="your-channel-id"
export WORKFLOW_DISCORD_MENTION="<@1465264645320474637>"  # optional
```
...
Set up Slack notifications so workflow can message you when sessions end, need input, or complete background tasks.
...
3. Name your app (e.g., "workflow Notifier") and select your workspace
...
curl -s -o /dev/null -w "%{http_code}" \
  -H "Content-Type: application/json" \
  -d "{\"text\": \"${MENTION_PREFIX}workflow test notification - Slack is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  WORKFLOW_SLACK_WEBHOOK_URL=https://hooks.slack.com/services/...
  WORKFLOW_SLACK_MENTION=<@U1234567890>
...
```bash
export WORKFLOW_SLACK_WEBHOOK_URL="https://hooks.slack.com/services/T00/B00/xxx"
export WORKFLOW_SLACK_MENTION="<@U1234567890>"  # optional
```
...
- `workflow --telegram` — Activates Telegram notifications (sets `WORKFLOW_TELEGRAM=1`)
- `workflow --discord` — Activates Discord notifications (sets `WORKFLOW_DISCORD=1`)
- `workflow --slack` — Activates Slack notifications (sets `WORKFLOW_SLACK=1`)
- `workflow --webhook` — Activates webhook notifications (sets `WORKFLOW_WEBHOOK=1`)
- `workflow --openclaw` — Activates OpenClaw gateway integration (sets `WORKFLOW_OPENCLAW=1`)
...
**Examples:**
- `workflow --telegram --discord` — Telegram + Discord active
- `workflow --telegram --slack --webhook` — Telegram + Slack + Webhook active
- `workflow --telegram --openclaw` — Telegram + OpenClaw active
- `workflow` — No notifications sent (all platforms require explicit activation)

---

## Hook Event Templates

Customize notification messages per event and per platform using `WORKFLOW_config.hook.json`.
...
Check if `${AGENT_CONFIG_DIR:-~/.agent}/WORKFLOW_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Config file: ~/.agent/WORKFLOW_config.hook.json
```
...
**Options:**
1. **session-end** - When a Cursor Agent session finishes (most common)
2. **ask-user-question** - When Cursor Agent is waiting for input
3. **session-idle** - When Cursor Agent finishes and waits for input
...
RAW FIELDS:
  ...
  {{activeMode}}     - Active workflow mode name
...
Read or create `${AGENT_CONFIG_DIR:-~/.agent}/WORKFLOW_config.hook.json` and merge the new settings:
...
If `~/.agent/WORKFLOW_config.openclaw.json` exists, detect and offer migration:

**Step 1: Detect Legacy Config**
```bash
LEGACY_CONFIG="${AGENT_CONFIG_DIR:-$HOME/.agent}/WORKFLOW_config.openclaw.json"
...
If migrate:
- Read `WORKFLOW_config.openclaw.json`
- Transform to custom integration format
- Save to `.workflow-config.json`
- Backup legacy to `WORKFLOW_config.openclaw.json.bak`
...
curl -X POST \
  ...
  -d '{"event":"test","instruction":"workflow test notification","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
  "$WEBHOOK_URL"
```
...
Merge into `.workflow-config.json`:
...
| `{{tmuxSession}}` | tmux session name | `Cursor Agent:my-project` |
