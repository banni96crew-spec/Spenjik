...
```
/science-workflow <goal>                    # Standard research with user checkpoints
/science-workflow AUTO: <goal>              # Fully autonomous until complete
/science-workflow status                    # Check current research session status
/science-workflow resume                    # Resume interrupted research session
/science-workflow list                      # List all research sessions
/science-workflow report <session-id>       # Generate report for session
```

### Quick Examples

```
/science-workflow What are the performance characteristics of different sorting algorithms?
/science-workflow AUTO: Analyze authentication patterns in this codebase
/science-workflow How does the error handling work across the API layer?
```
...
Delegate(role="scientist", tier="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Delegate(role="scientist", tier="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Delegate(role="scientist", tier="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Delegate(role="scientist", tier="sonnet", prompt="
...
```
/science-workflow AUTO: Comprehensive security analysis of the authentication system
...
Delegate(role="scientist", tier="haiku", prompt="[STAGE:1] Analyze src/api/...")
Delegate(role="scientist", tier="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Delegate(role="scientist", tier="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Delegate(role="scientist", tier="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Delegate(role="scientist", tier="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Delegate(role="scientist", tier="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Delegate(role="scientist", tier="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/science-workflow status` | Show current session progress |
| `/science-workflow resume` | Resume most recent interrupted session |
| `/science-workflow resume <session-id>` | Resume specific session |
| `/science-workflow list` | List all sessions with status |
| `/science-workflow report <session-id>` | Generate/regenerate report |
| `/science-workflow cancel` | Cancel current session (preserves state) |
...
Progress is preserved in `.workflow/research/{session-id}/` for resume.
