...
name: adopt
description: >-
  Brownfield onboarding — audits existing project artifacts for template format compliance (not just
  existence), classifies gaps by impact, and produces a numbered migration plan. Use when joining an
  in-progress project or upgrading from an older template version. Distinct from project-stage-detect
  (which checks what exists) — this checks whether what exists will actually work with the template's
  skills.
---
...
- Treat user-provided arguments as: `[focus: full | gdds | adrs | stories | infra]`.
...
**Audit mode:** `the first user-provided argument` (blank = `full`)
...
and offer to handle it immediately using asking the user. Choose the first
branch that applies:
