...
metadata:
  ...
  handoff: '.workflow/specs/deep-dive-{slug}.md'
---
...
Present the 3 hypotheses to the user via `a concise user question` for confirmation (1 round only):
...
Run the trace autonomously using the `trace` skill's behavioral contract.

### Team Mode Orchestration

Use **Cursor Agent built-in team mode** to run 3 parallel tracer lanes:
...
3. Each tracer worker must:
   ...
   - For **Lane 3: Misplacement / SoT Violation** findings, classify every candidate MOVE destination with `ownership_scope` before ranking recommendations:
     - `personal-config`: user-scope dotfiles, `[$AGENT_CONFIG_DIR|~/.agent]/`, personal repositories, or user-only agent rules
...
Save to `.workflow/specs/deep-dive-trace-{slug}.md`:
...
After saving:
- Persist `trace_path` in state: `state_write` with `state.trace_path = ".workflow/specs/deep-dive-trace-{slug}.md"`
- Keep any ephemeral trace/interview scratch artifacts under `.workflow/state/` or `state_write`; do not write temporary files to the repo root or arbitrary working paths.
...
- Save to `.workflow/specs/deep-dive-{slug}.md`
- Persist `spec_path` in state: `state_write` with `state.spec_path = ".workflow/specs/deep-dive-{slug}.md"`
...
- **Set up issue/branch/worktree first (Recommended)**
  ...
  - Action: Invoke the known project setup skill or workflow if one is named in guidance; otherwise invoke `/project-session-manager` with `spec_path` and the pre-flight findings as context. After setup completes, rerun this Phase 5 pre-flight before showing execution options.
...
If the guidance gate does not apply, or the pre-flight passes, present execution options via `a concise user question`:
...
1. **Ralplan → Autopilot (Recommended)**
   ...
   - Action: Invoke `/plan` with `--consensus --direct` flags and the spec file path (`spec_path` from state) as context. The `--direct` flag skips the workflow-plan skill's interview phase (the deep-dive interview already gathered requirements), while `--consensus` triggers the Planner/Architect/Critic loop. When consensus completes and produces a plan in `.workflow/plans/`, invoke `/autopilot` with the consensus plan as Phase 0+1 output — autopilot skips both Expansion and Planning, starting directly at Phase 2 (Execution).
   - Pipeline: `deep-dive spec → workflow-plan --consensus --direct → autopilot execution`

2. **Execute with autopilot (skip ralplan)**
   - Description: "Full autonomous pipeline — planning, parallel implementation, QA, validation. Faster but without consensus refinement."
   - Action: Invoke `/autopilot` with the spec file path as context. The spec replaces autopilot's Phase 0 — autopilot starts at Phase 1 (Planning).
...
**IMPORTANT:** On execution selection, **MUST** invoke the chosen skill via `slash-skill invocation` with explicit `spec_path`. Do NOT implement directly. The deep-dive skill is a requirements pipeline, not an execution agent.
...
<Tool_Usage>
- Use `a concise user question` for lane confirmation (Phase 2) and each interview question (Phase 4)
- Use `Agent(subagent_type="explore", model="haiku")` for brownfield codebase exploration (Phase 1)
- Use Cursor Agent built-in team mode for 3 parallel tracer lanes (Phase 3)
...
- Use `Write` tool to save trace result to `.workflow/specs/deep-dive-trace-{slug}.md` and final spec to `.workflow/specs/deep-dive-{slug}.md`; use `.workflow/state/` or `state_write` for ephemeral artifacts
- Run the Phase 5 workflow pre-flight before execution options when project guidance requires issue/branch/worktree setup
- Use `slash-skill invocation` to bridge to execution modes (Phase 5) — never implement directly
...
[Phase 5] Spec ready. User selects ralplan → autopilot.
  → workflow-plan --consensus --direct runs on the spec
...
- [ ] Phase 2 confirms hypotheses via a concise user question (1 round)
- [ ] Phase 3 runs trace with 3 parallel lanes (team mode, sequential fallback)
- [ ] Phase 3 saves trace result to `.workflow/specs/deep-dive-trace-{slug}.md` with per-lane critical unknowns
...
- [ ] Final spec saved to `.workflow/specs/deep-dive-{slug}.md` in standard deep-interview format
...
- [ ] Phase 5 "Ralplan → Autopilot" option explicitly invokes autopilot after workflow-plan consensus completes
...
- [ ] `slug`, `trace_path`, `spec_path` persisted in state for resume resilience; ephemeral artifacts stayed under `.workflow/state/` or `state_write`
</Final_Checklist>
...
Deep-dive's output (`.workflow/specs/deep-dive-{slug}.md`) feeds into the standard workflow pipeline:

```
/deep-dive "problem"
  → Trace (3 parallel lanes) + Interview (Socratic Q&A)
  → Spec: .workflow/specs/deep-dive-{slug}.md

  → /plan --consensus --direct (spec as input)
    → Planner/Architect/Critic consensus
    → Plan: .workflow/plans/ralplan-*.md
...
The execution bridge passes `spec_path` explicitly to downstream skills. autopilot/Ralph/team receive the path as a slash-skill invocation argument, so filename-pattern matching is not required.
