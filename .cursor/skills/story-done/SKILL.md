...
name: story-done
description: >-
  End-of-story completion review. Reads the story file, verifies each acceptance criterion against the
  implementation, checks for GDDADR deviations, prompts code review, updates story status to Complete,
...
- Treat user-provided arguments as: `[story-file-path] [--review full|lean|solo]`.
...
Batch up to 4 manual verification questions into a single asking the user call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Manual confirmation**: if the criterion was verified by asking the user
...
After completing the deviation checks in Phase 4, spawn `qa-lead` as a specialist review pass using gate **QL-TEST-COVERAGE** (`.cursor/docs/director-gates.md`).
...
- `lean` → Ask the user before proceeding:
...
Spawn `lead-programmer` as a specialist review pass using gate **LP-CODE-REVIEW** (`.cursor/docs/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them by asking the user:
