...
description: >-
  Map test coverage to GDD critical paths, identify fixed bugs without regression tests, flag coverage
  drift from new features, and maintain testsregression-suite.md. Use after implementing a bug fix or
...
- Treat user-provided arguments as: `[update | audit | report]`.
...
- `/regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, Ask the user:
