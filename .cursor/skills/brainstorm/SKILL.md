...
name: brainstorm
description: >-
  Guided game concept ideation — from zero idea to a structured game concept document. Uses
  professional studio ideation techniques, player psychology frameworks, and structured creative
  exploration.
---
...
- Treat user-provided arguments as: `[genre or theme hint, or 'open'] [--review full|lean|solo]`.
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Ask the user** at key decision points throughout brainstorming:
   ...
   asking the user to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an asking the user with preset options.)*
...
Bundle these into a single multi-tab asking the user with these exact tab labels:
...
Present all three. Then Ask the user to capture the selection.

**CRITICAL**: This must be a plain list prompt: no tabs, no form fields.
...
Do NOT use a tabbed or multi-field form here. This is a plain prompt with a short options list.
...
Ask these as asking the user calls — derive the options from the chosen concept, don't hardcode them:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` as a specialist review pass in parallel before moving to Phase 5. Issue both specialist review passes simultaneously — do not wait for one before starting the other.**
...
Collect both verdicts, then present them together using a two-tab asking the user:
...
**After identifying biggest technical risks, spawn `technical-director` as a specialist review pass using gate TD-FEASIBILITY (`.cursor/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` as a specialist review pass using gate PR-SCOPE (`.cursor/docs/director-gates.md`).**
...
If [B]: ask which section to revise using asking the user with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`
...
> to `design/gdd/game-concept.md`. Open a fresh Cursor agent chat to continue
