...
This is a Cursor-native game studio workflow for agentic game development. It organizes reusable Cursor Skills, project rules, specialist role guidance, and production documents into a studio-style process for moving from concept through release.

The workflow supports Godot, Unity, and Unreal projects. It keeps durable decisions in files, uses Cursor Skills for repeatable workflows, and uses specialist review passes when a design, architecture, production, or release decision needs domain critique.

## How to Use

### 1. Understand the Hierarchy

There are three tiers of specialist roles:

- **Tier 1 (high-reasoning model tier)**: Directors who make high-level decisions
...
- **Tier 2 (balanced model tier)**: Department leads who own their domain
  - `game-designer`, `lead-programmer`, `art-director`, `audio-director`,
    `narrative-director`, `qa-lead`, `release-manager`, `localization-lead`

- **Tier 3 (balanced or fast model tier)**: Specialists who execute within their domain
  - Designers, programmers, artists, writers, testers, engineers

### 2. Pick the Right Role or Subagent for the Job

Ask yourself: "What department would handle this in a real studio?"

| I need to... | Use this specialist role or skill |
|-------------|---------------|
...
### 3. Use Cursor Skills for Common Tasks
...
| `/dev-story` | Read a story and implement it — routes to the correct programmer role or subagent |
...
Templates are in `.cursor/docs/templates/` when that optional template folder is present:
...
Also in `.cursor/docs/templates/collaborative-protocols/` when present (used by skills, not typically edited directly):
...
1. Work flows through the hierarchy: Directors -> Leads -> Specialists
2. Conflicts escalate up the hierarchy
3. Cross-department work is coordinated by the `producer`
4. Specialist roles do not modify files outside their domain without explicit delegation
...
2. **Set up the engine** — Run `/setup-engine` (uses the brainstorm recommendation)
   - Updates Cursor project rules and technical preferences, detects knowledge gaps, and populates reference docs
   - Updates `.cursor/docs/technical-preferences.md` with naming conventions,
...
.cursor/
  ...
  docs/
    ...
    technical-preferences.md       -- Project-specific standards (updated by setup-engine)
    coding-standards.md            -- Coding and design doc standards
    coordination-rules.md          -- Specialist coordination rules
    context-management.md          -- Context and recovery practices
    directory-structure.md         -- Project directory layout
    workflow-catalog.yaml          -- Phase pipeline definition (read by help)
    setup-requirements.md          -- System prerequisites
    cursor-local-template.md       -- Personal preference template
