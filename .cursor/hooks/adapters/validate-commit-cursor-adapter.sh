#!/usr/bin/env bash
# Cursor adapter for validate-commit.sh.
...
TARGET="$HOOKS_DIR/validate-commit.sh"
...
if [ -z "$COMMAND" ]; then
  echo "Cursor adapter warning: could not extract shell command for validate-commit.sh; allowing execution." >&2
  exit 0
fi
...
if [ "$STATUS" -ne 0 ]; then
  echo "Cursor adapter warning: validate-commit.sh exited with $STATUS; allowing execution." >&2
fi
