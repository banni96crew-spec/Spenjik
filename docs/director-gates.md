...
```
Run `creative-director` as a specialist review pass using gate **CD-PILLARS** from
...
Run `[role-name]` as a specialist review pass:
...
Run all [N] specialist review passes in parallel before
...
Role: `creative-director` | Model tier: high-reasoning | Domain: Vision, pillars, player experience
...
Role: `technical-director` | Model tier: high-reasoning | Domain: Architecture, engine risk, performance
...
Role: `producer` | Model tier: high-reasoning | Domain: Scope, timeline, dependencies, production risk
...
Role: `art-director` | Model tier: balanced | Domain: Visual identity, art bible, visual production readiness
...
specialist's feasibility sign-off is needed. Tier 2 leads use the balanced model tier by default.
...
```
Run in parallel (issue all specialist review passes before waiting for any result):
